...
public class SVD {
   ...
   private final double EPS = 1.110223024E-16;
   
   private double g;
   private double[] u1 = new double[9];
   private double[] v1 = new double[9];
   private double[] t1 = new double[9];
   private double[] t2 = new double[9];

   private double[] tmp = t1;
   private double[] single_values = t2;

   private double[] rot = new double[9];
   private double[] e = new double[3];
   private double[] scales = new double[3];

   public void compute(double[] m, double[] outScale, double[] outRot) {
      int i;
   ...
   private double[] rotn = new double[9];

   private void svdReorder(double[] m, double[] t1, double[] t2, double[] scales, double[] outRot, double[] outScale) {
      ...
      if (almostEqual(Math.abs(scales[0]), Math.abs(scales[1])) && almostEqual(Math.abs(scales[1]), Math.abs(scales[2]))) {
         ...
         */
         ...
         index = out[in0] + 3;
         outRot[0 + 3] = rotn[index];

         index = out[in0] + 6;
         outRot[0 + 6] = rotn[index];

         index = out[in1];
         outRot[1] = rotn[index];

         index = out[in1] + 3;
         outRot[1 + 3] = rotn[index];

         index = out[in1] + 6;
         outRot[1 + 6] = rotn[index];

         index = out[in2];
         outRot[2] = rotn[index];

         index = out[in2] + 3;
         outRot[2 + 3] = rotn[index];

         index = out[in2] + 6;
         outRot[2 + 6] = rotn[index];
      }
   }
   
   private double[] cosl = new double[2];
   private double[] cosr = new double[2];
   private double[] sinl = new double[2];
   private double[] sinr = new double[2];
   ...
   private double d_sign(double a, double b) {
      double x;
      x = (a >= 0 ? a : -a);
      return (b >= 0 ? x : -x);
   }

   private double compute_shift(double f, double g, double h) {
      double d__1, d__2;
   ...
   private int compute_2X2(double f, double g, double h, double[] single_values, double[] snl, double[] csl, double[] snr, double[] csr, int index) {
   ...
   private double compute_rot(double f, double g, double[] sin, double[] cos, int index, int first) {
      double cs, sn;
      int i;
      double scale;
      int count;
   ...
   private void mat_mul(double[] m1, double[] m2, double[] m3) {
      int i;
   ...
   private void transpose_mat(double[] in, double[] out) {
      out[0] = in[0];
      out[1] = in[3];
      out[2] = in[6];
   ...
   private final boolean almostEqual(double a, double b) {
      if (a == b)
         return true;
